...
|                             | SGE (qsub)	                                   | Slurm (sbatch)                                              |
|:----------------------------|:-----------------------------------------------|:------------------------------------------------------------|
|Script directive	          | #$	                                           | #SBATCH                                                     |
|Job name	                  | -N <name>	                                   | --job-name=<name>                                           |
|Wall time limit	          | -l h_rt=<seconds>	                           | --time=<hh:mm:ss>                                           |
|Node count	                  | N/A	                                           | --nodes=<count>                                             |
|Processor count per node     | -pe <count>	                                   | --ntasks-per-node=<count>                                   |
|Memory limit	              | -l mem_free=<limit>	                           | --mem=<limit>                                               |
|Minimum memory per processor | N/A	                                           | --mem-per-cpu=<memory>                                      |
...
|Job dependency               | -hold_jid [jobID|jobname]                      | --dependency=after:jobID[:jobID...]                         |
...
|Defer job until time         | -a [YYMMDDhhmm]	                               | --begin=<date/time>                                         |
...
More information about the Slurm commands, flags and environment variables, can be found in the 
[Slurm web page](https://slurm.schedmd.com){target=blank}.

## Advanced. Submitting jobs with Slurm, good practice.

### What is SLURM?
Slurm is the application used by Kelvin-2 and other computing clusters to organize the job queues. Slurm distributes the jobs according to the resources requested by the users. Slurm also takes in account the computation resources that each user has already used, and it gives higher priority to users with the least used resources, looking for a fair distribution of those computation resources.
...
A list of the most common Slurm commands can be found in the [previous section](Job Submission.md#slurm-cheat-sheet).<br />
