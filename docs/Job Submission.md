...
The following GPU Hardware resources are available on Kelvin2.
...
Compilation should be done in the latest hardware, so one of the A100 GPU nodes. The drivers to be uploaded are the Nividia-CUDA version 11.7. These drivers are compatible with older hardware, and they will make the application to work in all the GPUs installed in Kelvin-2.
